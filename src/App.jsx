...
import Logo from "./imgs/logo3.webp";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const fetchEthPrice = async () => {
      ...
      try {
        ...
        const price = parseFloat(json?.ethereum?.usd);
  ...
  const fetchToken = async () => {
    ...
    try {
      ...
      if (!MORALIS_API_KEY) {
        setMessage(
          "Moralis API key missing. Add VITE_MORALIS_API_KEY to your .env",
        );
        return;
      }
      ...
      const [metadataResult, priceResult, dexResult] = await Promise.allSettled(
        [
          ...
          fetchWithRetry(() =>
            axios.get(
              `https://api.dexscreener.com/latest/dex/tokens/${checksummedAddr}`,
              { timeout: 10000 },
            ),
          ),
        ],
      ...
      if (dexResult.status === "fulfilled") dexData = dexResult.value?.data;
      ...
      if (dexData?.pairs && Array.isArray(dexData.pairs)) {
        ...
        if (basePairs.length > 0) {
          bestPair = basePairs.reduce((prev, cur) =>
            (Number(cur?.liquidity?.usd) || 0) >
            (Number(prev?.liquidity?.usd) || 0)
              ? cur
              : prev,
          );
      ...
      if (err.message?.includes("not found"))
        ...
        errorMsg += "Invalid Moralis API key.";
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              {ethPrice && (
                <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg border border-white/[0.06] bg-white/[0.03] hover:border-cyan-500/20 transition-all duration-200 cursor-default">
                  <span className="relative flex h-1.5 w-1.5 shrink-0">
                    <span
                      className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${isConnected ? "bg-emerald-400" : "bg-slate-500"}`}
                    />
                    <span
                      className={`relative inline-flex h-1.5 w-1.5 rounded-full ${isConnected ? "bg-emerald-400" : "bg-slate-500"}`}
                    />
                  </span>
              ...
              {isConnected && shortAddress && (
                ...
                >
                  ...
                  <span
                    className={`font-mono text-[11px] font-semibold tabular-nums ${isBase ? "text-cyan-400" : "text-red-400"}`}
                  >
                    {shortAddress}
                  </span>
                  {isBase && (
      ...
      <main className="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-6 sm:py-8 lg:py-14">
        ...
        {isConnected && (
          <div className="mb-6 flex flex-wrap items-center gap-3 px-4 py-3 rounded-xl bg-cyan-400/[0.02] border border-cyan-400/[0.07]">
            ...
            >
              <div
                className={`w-1.5 h-1.5 rounded-full shrink-0 ${isBase ? "bg-blue-400" : "bg-red-400"}`}
              />
              <span
                className={`font-mono text-[10px] font-semibold tracking-wider uppercase ${isBase ? "text-blue-400" : "text-red-400"}`}
              >
            ...
            <div className="relative rounded-2xl overflow-hidden border border-white/[0.06] bg-white/[0.02] backdrop-blur-xl">
              ...
              <div className="p-6 sm:p-5 md:p-6 lg:p-7">
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="shrink-0 w-10 h-10 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center bg-cyan-500/[0.08] border border-cyan-400/[0.12]">
                      <svg
                        className="w-5 h-5 sm:w-4 sm:h-4 text-cyan-400"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                  ...
                  <div className="hidden sm:flex shrink-0 items-center gap-1.5 px-2.5 py-1 rounded-lg ml-3 bg-blue-600/[0.08] border border-blue-500/[0.15]">
                    ...
                    <span className="font-mono text-[9px] text-blue-400 font-semibold tracking-[0.2em] uppercase">
                      Base
                    </span>
                  </div>
                </div>
                ...
                <button
                  ...
                  className={`relative w-full py-4 sm:py-3 rounded-xl font-mono text-sm sm:text-xs font-bold
                              tracking-wider sm:tracking-[0.15em] uppercase transition-all duration-300
                              active:scale-[0.98] flex items-center justify-center gap-2.5 select-none overflow-hidden
                              ${
                ...
                >
                  ...
                  {loading ? (
                    ...
                    <>
                      <svg
                        className="relative w-4 h-4 sm:w-3.5 sm:h-3.5 shrink-0"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                ...
                <div className="mt-4 pt-4 flex items-start gap-2 text-slate-600 border-t border-white/[0.04]">
                  <svg
                    className="w-3.5 h-3.5 sm:w-3 sm:h-3 shrink-0 mt-0.5 text-slate-700"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                  ...
                  <p className="font-mono text-[11px] sm:text-[10px] leading-relaxed tracking-wide">
                    Paste a valid Base ERC-20 address. Data sourced live from
                    Moralis + DexScreener.
                  </p>
                </div>
              </div>
            ...
            {!tokenData && !loading && (
              <div className="flex flex-col items-center justify-center px-6 py-14 sm:py-16 lg:py-20 rounded-2xl text-center bg-white/[0.01] border border-dashed border-white/[0.06]">
                <div className="w-14 h-14 sm:w-12 sm:h-12 rounded-2xl flex items-center justify-center mb-4 bg-white/[0.03] border border-white/[0.06]">
                  <svg
                    className="w-6 h-6 sm:w-5 sm:h-5 text-slate-700"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                <p className="font-display text-base sm:text-sm font-semibold text-slate-600 mb-1">
                  No Token Selected
                </p>
          ...
          {[
            {
              ...
              hover: "hover:border-cyan-400/[0.15]",
            ...
            ({ iconPath, iconColor, iconBg, shimmer, hover, label, desc }) => (
              <div
                key={label}
              ...
              >
                <div
                  className={`absolute top-0 left-0 right-0 h-px opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-r from-transparent ${shimmer} to-transparent`}
                />
                <div
                  className={`flex items-center justify-center w-11 h-11 sm:w-10 sm:h-10 rounded-xl mb-4 border transition-all duration-300 group-hover:scale-110 ${iconBg}`}
                >
                  <svg
                ...
                <h3 className="font-display text-base sm:text-sm font-bold text-white mb-2 tracking-tight">
                  {label}
                </h3>
                <p className="font-mono text-xs sm:text-[11px] text-slate-500 leading-relaxed tracking-wide">
                  {desc}
                </p>
              </div>
            ),
